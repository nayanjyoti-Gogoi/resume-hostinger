...
const ResumeBuilder = () => {
  ...
  const handleResumeSizeChange = (size) => {
    ...
    if (resumePreview) {
      ...
      resumePreview.classList.remove('size-a4', 'size-letter', 'size-legal');
      // Add the selected size class (always apply a size class)
  ...
  const downloadResume = () => {
    try {
      ...
      textElements.forEach(el => {
        ...
        el.style.wordSpacing = 'normal'; // Ensure proper word spacing
        el.style.letterSpacing = 'normal'; // Ensure proper letter spacing
      ...
      styleTag.textContent = `
        * {
        ...
        /* Fix for specific elements with spacing issues */
        .contact-item, .item-title, .resume-title, .company, .institution, .location {
      ...
      setTimeout(() => {
        html2canvas(resumeClone, {
          ...
          onclone: function(clonedDoc) {
            ...
            const allTextElements = clonedDoc.querySelectorAll('.resume-preview p, .resume-preview div, .resume-preview span, .contact-item, .item-title, .item-subtitle');
            allTextElements.forEach(el => {
              // Apply to all text elements, with special attention to contact items
              ...
              // Add extra spacing for contact items and titles
              if (el.classList.contains('contact-item') || 
                  el.classList.contains('item-title') || 
            ...
            // Fix Blue Sidebar template layout for PDF export
            const blueSidebarTemplate = clonedDoc.querySelector('.resume-preview[data-template="blue-sidebar"]');
            if (blueSidebarTemplate) {
              // Apply styles to the preview container
              blueSidebarTemplate.style.width = '100%';
              blueSidebarTemplate.style.maxWidth = '100%';
              blueSidebarTemplate.style.padding = '0';
              blueSidebarTemplate.style.margin = '0';
              blueSidebarTemplate.style.boxShadow = 'none';
              blueSidebarTemplate.style.borderRadius = '0';
              
              const resumeDoc = blueSidebarTemplate.querySelector('.resume-document');
              if (resumeDoc) {
                resumeDoc.style.width = '100%';
                resumeDoc.style.maxWidth = '100%';
                resumeDoc.style.padding = '0';
                resumeDoc.style.margin = '0';
                resumeDoc.style.boxShadow = 'none';
                resumeDoc.style.border = 'none';
                
                const sidebar = resumeDoc.querySelector('.resume-sidebar');
                if (sidebar) {
                ...
                const main = resumeDoc.querySelector('.resume-main');
                if (main) {
                  main.style.width = '62%';
                  main.style.padding = '15px 10px';
                  main.style.margin = '0';
                }
              }
            }
    ...
    try {
      ...
      setTimeout(() => {
        html2canvas(resumeClone, {
          ...
          onclone: function(clonedDoc) {
            // Apply additional fixes to the cloned document
            const allTextElements = clonedDoc.querySelectorAll('.resume-preview p, .resume-preview div, .resume-preview span');
            allTextElements.forEach(el => {
              el.style.wordSpacing = '0.05em';
              el.style.letterSpacing = '0.01em';
            });

            // Fix for Blue Sidebar template
            if (selectedTemplate === 'blue-sidebar') {
            ...
            // Fix for Split Circle template
            if (selectedTemplate === 'split-circle') {
              const resumeLeft = clonedDoc.querySelector('.resume-left');
              const resumeRight = clonedDoc.querySelector('.resume-right');
              const photoContainer = clonedDoc.querySelector('.photo-container');
              const photoCircle = clonedDoc.querySelector('.photo-circle');
              const resumePhoto = clonedDoc.querySelector('.resume-photo');
              
              if (resumeLeft && resumeRight) {
                resumeLeft.style.width = '40%';
                resumeLeft.style.padding = '30px 20px';
                resumeRight.style.width = '60%';
                resumeRight.style.padding = '30px 20px';
              }
              
              if (photoContainer && photoCircle && resumePhoto) {
                photoContainer.style.height = '350px';
                photoCircle.style.top = '-100px';
                photoCircle.style.right = '-100px';
                photoCircle.style.width = '400px';
                photoCircle.style.height = '400px';
                resumePhoto.style.top = '50px';
                resumePhoto.style.right = '50px';
                resumePhoto.style.width = '250px';
                resumePhoto.style.height = '250px';
              }
            }
          }
        }).then(canvas => {
                            ...
                            <div className="theme-color-option">
                              ...
                              <div className="d-flex align-items-center">
                                ...
                                <Form.Check
                                  ...
                                  name="theme-color"
                                  label="Black"
                                  checked={themeColor === '#000000'}
                                  onChange={() => handleThemeColorChange('#000000')}
                                />
                              </div>
                            </div>
                          </div>
